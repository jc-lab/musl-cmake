...
MUSL_EXPORT int getgrnam_r(const char *name, struct group *gr, char *buf, size_t size, struct group **res)
{
	return getgr_r(name, 0, gr, buf, size, res);
}

MUSL_EXPORT int getgrgid_r(gid_t gid, struct group *gr, char *buf, size_t size, struct group **res)
{
